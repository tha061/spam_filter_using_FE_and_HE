    ...
    */
    ...
    public:
        ...
        */
        ...
        /**
         * Tham add the dot product of two vectors
         * 
        */

       inline void total_sum_bfv(const Ciphertext &encrypted1, const Ciphertext &encrypted2, Ciphertext &encrypted_total_sum, 
                        const GaloisKeys &galois_keys, RelinKeys relin_keys, size_t slot_count)
        {
                        
            // cout << "Compute encrypted_result (encrypted_1 * encrypted_2)." << endl;
            Ciphertext encrypted_result, encrypted_result_rotate;
            multiply(encrypted1, encrypted2, encrypted_total_sum);
           // cout << relinearize the encrypted_total_sum to a ciphertext of size 2
            relinearize_inplace(encrypted_total_sum, relin_keys);
            ...
            int numbits = 0; 
            size_t slot_count_tm = slot_count;
            while (slot_count_tm > 0) { 
        
                // storing remainder in binary array 
                binaryNum[numbits] = slot_count_tm % 2; 
                slot_count_tm = slot_count_tm / 2; 
                // cout<<slot_count_tm<<endl;
                numbits++; 
            } 

           

            Ciphertext encrypted_rotate_tmp, encrypted_rotate_tmp1;
            // encrypted_total_sum = encrypted_result;    
            int e = 1;
            // cout << "\nOK\n";
            for (int j = numbits - 2; j > 0; j--)
            {
                // cout << "j = "<<j<<endl;
                // cout << "e = "<< e <<endl;
                size_t test = slot_count/2;
                // cout<< "slot_count/2 ="<< test <<endl;
                if (e > test - 1) 
                {
                    e = test - 1;
                    // cout << "e fixed = "<< e<<endl;
                }
                rotate_rows(encrypted_total_sum, -e, galois_keys, encrypted_rotate_tmp);
                add_inplace(encrypted_total_sum, encrypted_rotate_tmp);
                // cout << "\nOK"<<endl;
                e = 2*e;

                // cout << "binaryNum[j] = "<<binaryNum[j]<<endl;

                if (binaryNum[j] == 1)
                {
                    rotate_rows(encrypted_total_sum, -1, galois_keys, encrypted_rotate_tmp1);
                    add(encrypted_total_sum, encrypted_rotate_tmp1, encrypted_total_sum);
                    e = e + 1;
                }
        ...
        inline void total_sum(Ciphertext &encrypted_total_sum, 
                        const GaloisKeys &galois_keys, size_t slot_count)
        {
            // encrypted_total_sum = encrypted;
            ...
            for (int j = numbits - 2; j > 0; j--)
            {
                ...
                rotate_rows(encrypted_total_sum, -e, galois_keys, encrypted_rotate_tmp);//bfv
                // rotate_vector(encrypted_total_sum, -e, galois_keys, encrypted_rotate_tmp);//ckks
                add_inplace(encrypted_total_sum, encrypted_rotate_tmp);
                // cout << "\nOK"<<endl;
                e = 2*e;
                ...
                if (binaryNum[j] == 1)
                {
                    rotate_rows(encrypted_total_sum, -1, galois_keys, encrypted_rotate_tmp1);//bfv
                    // rotate_vector(encrypted_total_sum, -1, galois_keys, encrypted_rotate_tmp1);//ckks
                    add(encrypted_total_sum, encrypted_rotate_tmp1, encrypted_total_sum);
                    e = e + 1;
                }
        ...
        inline void total_sum_ckks(Ciphertext &encrypted_total_sum, 
                        const GaloisKeys &galois_keys, size_t slot_count)
        {
            // encrypted_total_sum = encrypted;
        ...
        */
        ...
        @throws std::invalid_argument if encrypted is not in the default NTT form
        @throws std::invalid_argument if the output scale is too large for the encryption parameters
        @throws std::invalid_argusent if pool is uninitialized
        @throws std::logic_error if result ciphertext is transparent
        */
        inline void square(
