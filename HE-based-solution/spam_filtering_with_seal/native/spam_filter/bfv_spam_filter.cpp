...
void spam_filter_HE()
{
    ...
    size_t poly_modulus_degree = 32768; //16384; //16384;//32768; //16384;  //8192;
    parms.set_poly_modulus_degree(poly_modulus_degree);
    parms.set_coeff_modulus(CoeffModulus::BFVDefault(poly_modulus_degree));
    parms.set_plain_modulus(PlainModulus::Batching(poly_modulus_degree, 60)); //parms.set_plain_modulus(1024);
    ...
    vector<int64_t> vector_0(slot_count, 0); 
    ...
    vector<int64_t> vector_1(slot_count, 0); 
    // for (int i=0; i < COLUMN_2; i++)
    // {
    //     for (int j=0; j < COLUMN_1; j++)
    //     {
    ...
    vector<int64_t> vector_3(slot_count, 0); 
    ...
    for (int i=0; i<COLUMN_1; i++)
    {
        evaluator.total_sum(encrypted_result_X_mul_weight1[i],galois_keys, slot_count);
    ...
    vector<int64_t> vector_bit_mask(slot_count, 0);
    for (int i=0; i<COLUMN_1; i++)
    {
        for (int j=0; j<slot_count;j++)
        {
    ...
    vector<int64_t> vector_bit_mask_w2(slot_count, 0);
    for (int i=0; i<COLUMN_2; i++)
    {
        for (int j=0; j<slot_count;j++)
        {
            vector_bit_mask_w2[j] = 0;
        }
        vector_bit_mask_w2[i] = 1;
    ...
    vector<int64_t> vector_bit_mask_w3(slot_count, 0);
    for (int i=0; i<COLUMN_3; i++)
    {
        for (int j=0; j<slot_count;j++)
        {
            vector_bit_mask_w3[j] = 0;
        }
        vector_bit_mask_w3[i] = 1;
    ...
    vector<int64_t> vector_bit_mask_w4(slot_count, 0);
    for (int i=0; i<COLUMN_4; i++)
    {
        for (int j=0; j<slot_count;j++)
        {
            vector_bit_mask_w4[j] = 0;
        }
        vector_bit_mask_w4[i] = 1;
