...
void plaintext_model_encr_input()
{
    print_example_banner("Test spam filter on encr email and plaintext model");
    ...
    ofstream myfile;
    myfile.open ("native/spam_filter/SF_results/clear_model_runtime_ceas08_vec_5000_5_pc.csv");
    ...
    string input_file_name = "native/spam_filter/all_datasets/ceas08-1_dataset/vec_5000/ceas08_X_test_vector_length_5000_entire_17-04-2021.csv";
    string w1_file_name = "native/spam_filter/all_datasets/ceas08-1_dataset/vec_5000/vec_5000_weight_1.csv";
    string w2_file_name = "native/spam_filter/all_datasets/ceas08-1_dataset/vec_5000/vec_5000_weight_2.csv";
    string w3_file_name = "native/spam_filter/all_datasets/ceas08-1_dataset/vec_5000/vec_5000_weight_3.csv";
    string w4_file_name = "native/spam_filter/all_datasets/ceas08-1_dataset/vec_5000/vec_5000_weight_4.csv";
    ...
    for (int iter=0; iter < iterations; iter++)
    {
        ...
        vector<Plaintext> plain_matrixes1(COLUMN_1);
        vector<Ciphertext> encrypted_weight_1(COLUMN_1);
        for (int i=0; i<COLUMN_1/2; i++)
        {
            // cout << "Encrypting weight_1[" << i << "]:" << endl;
            batch_encoder.encode(weight_1[i], plain_matrixes1[i]);
        ...
        trackTaskPerformance(time_track_list, "encoding weights (ms)", t1, t2);
...
#pragma omp parallel for shared() private(i)
        for (int i=0; i<COLUMN_1/2; i++)
        {
            ...
            evaluator.multiply_plain(encrypted_vector_X, plain_matrixes1[i], encrypted_result_X_mul_weight1[i]);
            evaluator.relinearize_inplace(encrypted_result_X_mul_weight1[i], relin_keys);
        }
    ...
    #pragma omp parallel for shared() private(i)
        ...
        {
            // cout<< "X multiply weight1["<<i<<"]"<<endl;
            evaluator.multiply_plain(masked_encrypted_X_mul_weight1_squared, plain_matrixes2[i], encrypted_result_X_mul_weight1_sqr_mul_weight2[i]);
