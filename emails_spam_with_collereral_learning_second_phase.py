...
file_name = "email_Prediction_output_%s_collateral_new_pri_model.txt" %d
...
### Need to specify the path to these npy files which we get from the .tflite quant model of the main task using the netron app:
FQuanFirstLayerWeight = "sequential_dense_MatMul_ReadVariableOp_transpose_IGpublabel.npy"
FQuanFirstLayerBias = "sequential_dense_MatMul_bias_IGpublabel.npy"
FQuanSecondLayerWeight = "sequential_dense_1_MatMul_ReadVariableOp_transpose_IGpublabel.npy"
FQuanSecondLayerBias = "sequential_dense_1_MatMul_bias_IGpublabel.npy"
...
### frezzing the layers that used in quadratic net: not be trained here
## modelPri = frozen(quad net) + MLP net
modelPri = keras.Sequential([
    keras.layers.InputLayer(input_shape=(first_layer_parameter,)),
    keras.layers.Dense(40),
    keras.layers.Lambda(lambda x: x * x),
    keras.layers.Dense(20),
    keras.layers.Dense(100,activation='tanh', name = '1layer'),
    keras.layers.Dense(20,activation='tanh', name = '2layer'),
    keras.layers.Dense(10,activation='tanh', name = '3layer'),
...
modelPri.fit(X_train_vector.toarray(), dummy_y_train_private, epochs=30, verbose=2)
loss, accuracy = modelPri.evaluate(X_test_vector.toarray(), dummy_y_test_private, verbose=2)

with open(file_name, 'a') as ff:
	print('Model_priv_summary:', file=ff)
	with redirect_stdout(ff):
		modelPri.summary()
...
#----------------------------------
# new model for main task: modelPub = frozen(quad net) + MLP net (MLP net can be different from the original model for main task)
...
with open(file_name, 'a') as ff:
	print('new_Model_pub_summary:', file=ff)
	with redirect_stdout(ff):
		modelPub.summary()

print('Accuracy of Public Learning when freezing quadratic net: %f' % (accuracy*100), file=open(file_name, "a"))
...
### combined model : frozen (MLP of main task) and frozen (MLP of private task), train the quad net again.
...
x6 = keras.layers.Dense(100,activation='tanh', name="thirdlayer")(x3)
x7 = keras.layers.Dense(20,activation='tanh', name="forthlayer")(x6)
x8 = keras.layers.Dense(10,activation='tanh', name="fifthlayer")(x7)
...
modelCombine.layers[6].set_weights(first_layer_weights_pub)  #first_pub
modelCombine.layers[7].set_weights(third_layer_weights)
modelCombine.layers[8].set_weights(second_layer_weights_pub)
modelCombine.layers[9].set_weights(forth_layer_weights)
#modelCombine.layers[10].set_weights(fifth_layer_weights)

modelCombine.compile(optimizer="adam",loss=['binary_crossentropy', 'categorical_crossentropy'],loss_weights = [2.0, -0.020], metrics = ['accuracy'])  # here alpha = 1
print("alpha: 0.01", file=open(file_name, "a"))
modelCombine.fit(X_train_vector.toarray(),[dummy_y_train,dummy_y_train_private], epochs=100)
...
## Pri model after combining
modelPriAftQ = keras.Model(inputs=inputs, outputs=x9)
modelPriAftQ.compile(optimizer="adam",loss='categorical_crossentropy',metrics=['accuracy'])
modelPriAftQ.layers[0].trainable = False
modelPriAftQ.layers[1].trainable = False
modelPriAftQ.layers[2].trainable = False
modelPriAftQ.layers[3].trainable = False
#modelPubAftQPri.layers[4].trainable = False
modelPriAftQ.layers[1].set_weights(QFirstLayer)
modelPriAftQ.layers[2].set_weights(QSecondLayer)
modelPriAftQ.layers[3].set_weights(QThirdLayer)
